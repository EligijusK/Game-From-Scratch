...
void Mesh::ReadMesh(char *filename)
{
    ...
    bool ret = tinyobj::LoadObj(&att, &shapes, &material, &warn, &err, filename, base_dir.c_str());
...
//    vec3 positions[4]; // add one on creation
//    vec2 textCoords[4];
//
...
//    std::cout << std::endl <<  sizeof (positions[0]);
//     vPos;
...
//    glBufferData(GL_ARRAY_BUFFER, numIndices * sizeof (indices[0]), indices, GL_STATIC_DRAW);
//    glBindVertexArray(0);
...
void Mesh::MeshInit()
{
    ...
    bool ret = tinyobj::LoadObj(&att, &shapes, &material, &warn, &err, "boxUv.obj", base_dir.c_str());
    ...
    //vec3 positions[numVertecis]; // add one on creation
        ...
        vec2 textCoords[meshData.size()];
        vec3 positions[meshData.size()];
        vec3 normals[meshData.size()];
        ...
        glGenBuffers(NUM_BUFFERS, m_vertexArrayBuffers);
        glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayBuffers[POSITION_VB]);
        glBufferData(GL_ARRAY_BUFFER,  meshData.size() * sizeof (positions[0]), positions, GL_STATIC_DRAW);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, m_vertexArrayBuffers[TEXCOORD_VB]);
        glBufferData(GL_ARRAY_BUFFER, meshData.size() * sizeof (textCoords[0]), textCoords, GL_STATIC_DRAW);
        ...
        glBufferData(GL_ARRAY_BUFFER,  meshData.size() * sizeof (normals[0]), normals, GL_STATIC_DRAW);


        char normal[] = "vNorm";
        GLuint vNorm = Shader::get_shader_index(normal);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof (indicesArray) * sizeof (indicesArray[0]), indicesArray, GL_STATIC_DRAW);
